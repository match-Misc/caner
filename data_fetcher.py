...
try:
    ...
    from models import XXXLutzChangingMeal, db  # Added Meal for data_loader
...
# Configure database from DATABASE_URL (Supabase)
...
def get_pdf(hesse_link, pdf_path):
    ...
    try:
        ...
        if geckodriver_path_from_env and os.access(geckodriver_path_from_env, os.X_OK):
            ...
            logger.warning(
                f"Geckodriver not found or executable in PATH. Trying fallback path: {geckodriver_path_snap}"
            )
            if os.path.exists(geckodriver_path_snap) and os.access(
                geckodriver_path_snap, os.X_OK
            ):
                logger.info(
                    f"Using geckodriver from fallback path: {geckodriver_path_snap}"
                )
                driver_service = FirefoxService(executable_path=geckodriver_path_snap)
                driver = webdriver.Firefox(service=driver_service, options=options)
            else:
                logger.error(
                    f"Geckodriver not found or not executable at fallback path: {geckodriver_path_snap}. Trying without explicit path."
                )
...
def download_and_manage_xxxlutz_vouchers():
    ...
    for item in os.listdir(vouchers_dir):
        if item.startswith("temp_voucher_") and item.endswith(".pdf"):
            ...
            try:
                ...
                logger.warning(
                    f"Could not remove old temp file {temp_file_to_remove}: {e}"
                )
    ...
    if not download_successful:
        logger.error(
            f"get_pdf failed for voucher URL {XXXLUTZ_VOUCHER_URL}. Checking existing voucher."
        )
        return (
            os.path.exists(new_voucher_path)
    ...
    try:
        if (
            ...
            logger.error(
                f"Downloaded voucher {temp_voucher_path} is missing or too small (Size: {os.path.getsize(temp_voucher_path) if os.path.exists(temp_voucher_path) else 'N/A'} bytes). Minimum: {MIN_VOUCHER_PDF_SIZE_BYTES}"
            )
        ...
        if not os.path.exists(new_voucher_path):
            ...
            if temp_voucher_size != new_voucher_size:
                logger.info(
                    "Downloaded voucher size differs from current 'neue_gutscheine.pdf'. Rotation needed."
                )
                need_rotation = True
            else:
                try:
                    ...
                    if (time.time() - new_voucher_mtime) > VOUCHER_MAX_AGE_SECONDS:
                        logger.info(
                            f"Voucher {new_voucher_path} is older than {VOUCHER_MAX_AGE_SECONDS // (24 * 60 * 60)} days. Rotation needed."
                        )
                        need_rotation = True
                    else:
                        logger.info(
                            f"Voucher {new_voucher_path} is up-to-date (size and age)."
                        )
                except Exception as e:
                    logger.warning(
                        f"Error checking age of {new_voucher_path}: {e}. Proceeding based on size difference."
                    )
        ...
        if need_rotation:
            ...
            try:
                ...
                logger.error(
                    f"CRITICAL: Failed to move {temp_voucher_path} to {new_voucher_path} after rotating: {e}"
                )
                return False
        else:
            logger.info(
                f"Removing temporary file {temp_voucher_path} as voucher is up-to-date."
            )
        ...
        if os.path.exists(temp_voucher_path):
            try:
                ...
                logger.error(
                    f"Error cleaning up {temp_voucher_path} in exception: {e_clean}"
                )
        return (
            os.path.exists(new_voucher_path)
            and os.path.getsize(new_voucher_path) >= MIN_VOUCHER_PDF_SIZE_BYTES
...
Ensure the meal descriptions are complete as seen on the menu under the 'Hauptspeise' section."""
    ...
    try:
        ...
        if not menu_date_str or not extracted_meals or len(extracted_meals) < 1:
            logger.warning(
                f"Mistral API did not return expected data structure or missing date/meals. Date: {menu_date_str}, Meals: {extracted_meals}"
            )
            return False
    ...
    with app.app_context():
        try:
            ...
            if existing_meal_count == 0:
                logger.info(
                    "No existing XXXLutz changing meals found. Meals will be updated from menu."
                )
                update_meals = True
            # If there are existing meals, update only if the menu date is not in the future (i.e., today or past).
            elif menu_date <= date.today():
                logger.info(
                    f"Menu date {menu_date_str} is today or in the past. DB has existing meals. Proceeding to update meals."
                )
                update_meals = True
            else:  # existing_meal_count > 0 AND menu_date > date.today() (menu is for the future)
                logger.info(
                    f"Menu date {menu_date_str} is in the future and DB is not empty. Meals will not be updated at this time."
                )
                update_meals = (
                    False  # Explicitly set to false, though it's initialized to false.
                )
        except Exception as e:
            logger.error(
                f"Database error checking existing meals: {e}. Cannot proceed with update check."
            )
            return False
    ...
    with app.app_context():
        try:
            ...
            db.session.commit()
            logger.info(
                f"Successfully updated {added_count} XXXLutz changing meals in the database."
            )
            return True
        except Exception as e:
...
def refresh_menu_hg_and_process():
    ...
    try:
        ...
        if (
            ...
            logger.error(
                f"Menu HG PDF {pdf_path} missing or too small after download attempt (Size: {os.path.getsize(pdf_path) if os.path.exists(pdf_path) else 'N/A'})."
            )
            if os.path.exists(pdf_path):
                os.remove(pdf_path)
            return False
            ...
            logger.info(
                f"Successfully downloaded Menu HG PDF to {pdf_path} (Size: {os.path.getsize(pdf_path)})."
            )
            pdf_downloaded_ok = True

        if (
            not pdf_downloaded_ok
        ):  # Should not be reached if previous checks return False, but as a safeguard
            return False
        ...
        try:
            ...
            if images:
                ...
                if (
                    ...
                    logger.info(
                        f"Successfully converted PDF to PNG: {png_path} (Size: {os.path.getsize(png_path)})."
                    )
                    png_converted_ok = True
                else:
                    logger.warning(
                        f"PNG file {png_path} is too small or missing after conversion (Size: {os.path.getsize(png_path) if os.path.exists(png_path) else 'N/A'})."
                    )
            else:
        ...
        if process_menu_image_and_update_meals(png_path):
            ...
            logger.error(
                "--- Menu HG Refresh Failed (AI processing or DB update step) ---"
            )
            return False

    except Exception as e:
        logger.error(
            f"Critical error during Menu HG refresh/process top-level function: {e}"
        )
        logger.error(traceback.format_exc())
